...
public final class MassSlideAnimator extends PropagatingAnimator<PropagatingAnimator.ViewHolder> implements Joinable {
    ...
    private final class SlidePropagation implements Propagation<ViewHolder> {
        ...
        private int getDistance(ViewHolder holder) {
            ...
            if(holder.mSide==androidx.recyclerview.R.styleable.RecyclerView_android_descendantFocusability){
                distance = (this.mWindowHeight - targetY) + Math.abs(epicenterX - targetX);

            }
            else if(holder.mSide== androidx.recyclerview.R.styleable.RecyclerView_layoutManager){
...
//            }
    ...
    final static class ViewHolder extends PropagatingAnimator.ViewHolder {
        ...
        private ViewHolder(MassSlideAnimator this$0, View view, ViewGroup root, Rect epicenter, Direction direction) {
            ...
            this.mSide = i;
            if (m1984xdbb8a697()[direction.ordinal()] == androidx.recyclerview.R.styleable.RecyclerView_android_descendantFocusability) {
                this.mStartY = getEndY(root);
                this.mEndY = 0.0f;
            } else if (m1984xdbb8a697()[direction.ordinal()] == androidx.recyclerview.R.styleable.RecyclerView_layoutManager) {
                this.mStartY = 0.0f;
...
//            }
        ...
        private float getEndY(ViewGroup root) {
            ...
            if (this.mSide == androidx.recyclerview.R.styleable.RecyclerView_android_descendantFocusability) {
                return ((float) (-root.getHeight())) / scaleFactor;

            } else if (this.mSide == androidx.recyclerview.R.styleable.RecyclerView_layoutManager) {
                return ((float) root.getHeight()) / scaleFactor;
...
//            }
    ...
    private MassSlideAnimator(Builder builder) {
        ...
        if(m1985xdbb8a697()[this.mDirection.ordinal()]==androidx.recyclerview.R.styleable.RecyclerView_android_descendantFocusability){
            super.setInterpolator(sSlideInInterpolator);

        }
            else if(m1985xdbb8a697()[this.mDirection.ordinal()]==androidx.recyclerview.R.styleable.RecyclerView_layoutManager){
